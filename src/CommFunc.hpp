...
#define SPARSE_FILE_TYPE_3 \
  3  // 16*uint32s + uint64_t + uint64_ts + uint32_ts + floats (indicator+samplesize+snpnumber+probenumber+ 6*-9s
     // +valnumber+cols+rowids+betases) [default]
#define DENSE_FILE_TYPE_3 \
  5  // 16*uint32s + floats (indicator+samplesize+snpnumber+probenumber+ 6*-9s + values) [default]
