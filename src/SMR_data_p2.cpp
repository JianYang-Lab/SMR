...
double calSE(double beta, double t_stat) {
    ...
    if (fformat == 0) {
      ...
      while (!flptr.eof() && !gzeof(gzfile)) {
        ...
        if (buf[0] != '\0') {
          ...
          if (vs_buf[0] == "NA" || vs_buf[0] == "na" || vs_buf[1] == "NA" || vs_buf[1] == "na" || vs_buf[2] == "NA" ||
              vs_buf[2] == "na") {
          ...
          if (vs_buf[3] == "NA" || vs_buf[4] == "NA") {
            printf(
                "WARNING: Allele infomation of row %d is \"NA\" in esd file \"%s\"! This may cause \
      ...
      if (snpinfo[l].snpchr == probchr && snpinfo[l].bp <= cisuperBounder && snpinfo[l].bp >= cislowerBounder) {
        ...
        while (startptr >= 0 && curChr == snpinfo[startptr].snpchr && transbp - snpinfo[startptr].bp <= trans_itvl) {
          if (fabs(snpinfo[startptr].se + 9) > 1e-6) {
            ...
            if (upperBp.size() > 0 && translowerBounder <= upperBp[upperBp.size() - 1] &&
                tran_chr[upperBp.size() - 1] == snpinfo[startptr].snpchr)  // trans region merges
            {
              merged = true;
    ...
    for (int l = 0; l < _rs.size(); l++) {
      ...
      if (iter != esi_map.end()) rsid[l] = iter->second;
        ...
        printf(
            ...
            "again. Otherwise please report this bug.\n",
            _rs[l].c_str());
        exit(EXIT_FAILURE);
      }
    }
...
void make_besd(char* outFileName, char* syllabusName, bool gctaflag, bool plinkflag, bool gemmaflag, bool merlinflag,
  ...
  for (int j = 0; j < esdfileNum; j++) {
    ...
    if (epiNum < epi_map.size()) {
      ...
      epi << prbiflst[j].probechr << '\t' << prbiflst[j].probeId << '\t' << 0 << '\t' << prbiflst[+j].bp << '\t'
          << prbiflst[j].genename << '\t' << prbiflst[j].orien << '\n';
      epiNum++;
    }
  }
  ...
  if (save_dense_flag) {
    ...
    save_slct_txts_sbesd(outFileName, esiNum, epiNum, epi2esd, prbiflst, fformat, esi_rs, esi_a1, esi_a2, cis_itvl,
...
void make_besd_fmat(char* fmatfileName, char* outFileName, bool mateqtlflag, bool fastnflag, bool fastpflag,
  ...
  int rspos = -9, prbpos = -9, betapos = -9, ppos = -9, cnum = -9, prbchrpos = -9, prbbppos = -9, prbstrandpos = -9,
      rschrpos = -9, rsbppos = -9, t_statpos = -9;
  if (mateqtlflag) {
...
void make_besd_byQfile(char* qfileName, char* outFileName, bool save_dense_flag, int cis_itvl, int trans_itvl,
  ...
  if (vs_buf.size() == 13) {
    printf(
        "ERROR: the input file could be in the old Query format. \
  ...
  while (fgets(buf, MAX_LINE_SIZE, qfile)) {
    ...
    if ((vs_buf[3] == "NA" || vs_buf[4] == "NA" || vs_buf[3] == "na" || vs_buf[4] == "na") && !warningdone) {
      printf(
          "WARING: Allele information of one or more SNPs is \"NA\". \
    ...
    if (!vs_buf[11].compare("NA")) {
      printf(
          "WARNING: this row is omitted because the effect size of the SNP \
                    is missing (\"NA\").\n");
      printf("%s\n", buf);
      continue;
    }
    ...
    if (iter != esi_map.end()) {
      ...
      if (tmpchr != esi_chr) {
        printf(
            "ERROR: the same SNP ID %s on different \
      ...
      if (atoi(vs_buf[2].c_str()) != esi_bp) {
        printf(
            "ERROR: multiple positions found for the \
      ...
      if (esi_a1 == vs_buf[3] && esi_a2 == vs_buf[4]) {
        ...
        printf(
            "WARING: switched ref allele with alt allele \
        ...
        printf(
            "ERROR: inconsistent alleles for the SNP %s \
    ...
    if (iter != epi_map.end()) {
      ...
      if (epi_chr != prbchr) {
        printf(
            "ERROR: the same probe ID %s found on different"
            "chromosomes.\n",
            vs_buf[6].c_str());
        exit(EXIT_FAILURE);
      }
      if (epi_bp != atoi(vs_buf[8].c_str())) {
        printf(
            "ERROR: multiple possions found for the same"
            "probe %s.\n",
            vs_buf[6].c_str());
        exit(EXIT_FAILURE);
      }
  ...
  if (save_dense_flag) {
    if (sparsity > 0.4) {
      ...
      for (int j = 0; j < epiNum; j++) {
        ...
        // init with -9
        for (int k = 0; k < bsize; k++) buffer[k] = -9;
  ...
  for (int i = 0; i < eqtlinfo->_include.size(); i++) {
    ...
    std::string str =
        ...
        (eqtlinfo->_epi_orien[eqtlinfo->_include[i]] == '*' ? "NA"
                                                            : atos(eqtlinfo->_epi_orien[eqtlinfo->_include[i]])) +
        '\n';
    if (fputs_checked(str.c_str(), efile)) {
      printf("ERROR: in writing file %s .\n", epiName.c_str());
      exit(EXIT_FAILURE);
