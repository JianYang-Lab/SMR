...
//
// Because r(i,j) = r(j,i), LD is computed and stored only for pairs with j > i (the upper
// triangle): each unordered in-window pair appears exactly once, in the earlier SNP's
// segment; a pair (i, j) with i > j is read back from SNP j's segment.
//
...
//
// ldWind is in Kb (converted to bp here). SNPs are sorted by (chr, bp). A single forward
// pointer p tracks the first SNP not within the current window, moving monotonically:
// for SNP i, ldnum_i = p - i - 1. Cost is O(n + total partners).
int get_ld_layout(bInfo* bdata, int ldWind, std::vector<std::uint64_t>& cols) {
  int n = 0, p = 0;
  long window = ldWind * 1000;
  int maxldnum = 0;
  const int N = bdata->_include.size();
  cols.resize(N + 1);
  for (int i = 0; i < N; i++) {
    const int snpi = bdata->_include[i];  // raw SNP index of the i-th included SNP
    const int chri = bdata->_chr[snpi];
    const int bpi = bdata->_bp[snpi];
    if (p <= i) p = i + 1;
    while (p < N) {
      const int snpp = bdata->_include[p];  // raw SNP index of the p-th included SNP
      const int chrp = bdata->_chr[snpp];
      const int bpp = bdata->_bp[snpp];
      if (chrp != chri || bpp - bpi > window) break;
      p++;
    }
    int ldnum = p - i - 1;
    cols[i + 1] = cols[i] + ldnum;
    maxldnum = std::max(maxldnum, ldnum);
  }
  // round (max ldnum + 1) up to the next power of two: 1 << (n+1)
  ++maxldnum;
...
void extract_snp_kb(bInfo* bdata, std::string rsnames, int windInKb) {
  ...
  for (int i = 0; i < bdata->_include.size(); i++) {
    ...
    if (bdata->_chr[tmpint] == snpchr && bdata->_bp[tmpint] >= lowbound && bdata->_bp[tmpint] <= upbound)
      snplist.push_back(bdata->_snp_name[tmpint]);
  }
  update_id_map_kp(snplist, bdata->_snp_name_map, bdata->_include);
...
void lookup(char* outFileName, char* bldFileName, char* snplstName, char* snplst2exclde, int chr, char* snprs,
  ...
  ld_esi_man(&ldinfo, snplstName, snplst2exclde, chr, snprs, fromsnprs, tosnprs, snpWind, snpWindflg, fromsnpkb,
             tosnpkb, snprs2exclde);
...
void get_ld_blk_pnt(bInfo* bdata, std::vector<int>& brk_pnt1, std::vector<int>& brk_pnt2, std::vector<int>& brk_pnt3,
  ...
  for (i = 1, j = 0; i < m; i++) {
    if (i == (m - 1)) {
      ...
      chr_start = true;
    } else if ((bdata->_bp[bdata->_include[i]] - bdata->_bp[bdata->_include[brk_pnt1[j]]] > wind_bp) &&
               (i - brk_pnt1[j] >= wind_snp)) {
      chr_start = false;
      brk_pnt1.push_back(i - 1);
      j++;
      brk_pnt1.push_back(i);
  ...
  for (i = 1; i < brk_pnt1.size() && brk_pnt1.size() > 2; i++) {
    if ((bdata->_chr[bdata->_include[brk_pnt1[i - 1]]] == bdata->_chr[bdata->_include[brk_pnt1[i]]]) &&
        (brk_pnt1[i] - brk_pnt1[i - 1] > 1)) {
...
void calcu_ld_blk_split(bInfo* bdata, int size, int size_limit, MatrixXf& X_sub, VectorXf& ssx_sqrt_i_sub,
...
void calcu_ld_blk(bInfo* bdata, std::vector<int>& brk_pnt, std::vector<int>& brk_pnt3, VectorXf& mean_rsq,
  ...
  for (i = 0; i < brk_pnt.size() - 1; i++) {
    ...
    if (size > size_limit)
      calcu_ld_blk_split(bdata, size, size_limit, X_sub, ssx_sqrt_i_sub, rsq_cutoff, rsq_size, mean_rsq_sub,
                         max_rsq_sub, s1, s2, second);
    else {
